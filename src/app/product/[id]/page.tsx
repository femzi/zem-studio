...
export default function ProductPage({
    params,
}: { params: Promise<{ id: string }> } & Partial<Props>) {
    const { id } = use(params);
    const [selectedSize, setSelectedSize] = useState("");
    const [isAdding, setIsAdding] = useState(false);

    const [product] = products.filter((e) => id === e.id);
    ...
    const addToCart = () => {
        ...
        const cartItem = {
            id: id, // keep as string for strict comparison
            name: title,
            price: price,
            quantity: 1,
            image: image,
            ...
            <div className="grid sm:grid-cols-2 grid-cols-1 gap-8 sm:gap-16 px-8 sm:px-16 md:px-32 md:py-12 py-4">
                ...
                <div className="flex-col space-y-2 flex">
                    ...
                    <div className="flex flex-col gap-4 py-8">
                        ...
                        {/* <div>
                            <Button className="w-full py-5 bg-amber-500 text-white tracking-widest rounded-none uppercase font-mono">
                                Buy It Now
                            </Button>
                        </div> */}
                    </div>
